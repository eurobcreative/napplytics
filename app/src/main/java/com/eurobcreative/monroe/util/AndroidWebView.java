...
//	private volatile int totalByte;
    public AndroidWebView(Context context, boolean spdyTest, WebViewProtocol protocol, long startTimeFilter, String url) {
        ...
        try {
            ...
            client = new OkHttpClient.Builder().sslSocketFactory(sslc.getSocketFactory()).build();
        ...
        if (client.connectionPool() != null && client.connectionPool().connectionCount() != 0) {
            client.connectionPool().evictAll();
        }

        //ALV client.connectionPool(ConnectionPool.getDefault());
//		client.getCache().delete();

        if (spdyTest) {
            if (protocol.equals(WebViewProtocol.HTTP)) {
                client.protocols().addAll(okhttp3.internal.Util.immutableList(Protocol.HTTP_1_1));
            } else {
                client.protocols().addAll(okhttp3.internal.Util.immutableList(Protocol.SPDY_3, Protocol.HTTP_1_1));
            }
        } else {
            client.protocols().addAll(okhttp3.internal.Util.immutableList(Protocol.HTTP_1_1));
        }
...
//		            AndroidWebView.this.destroy();
    ...
    class MyWebResourceResponse extends WebResourceResponse {
        ...
        public MyWebResourceResponse(String mimeType, String encoding, InputStream data) {
        ...
        public int read() throws IOException {
            if (!initialized) {
                try {
                    ...
                    //ALV Logger.d("ashkan_plt: HTTP: " + client.getConnectionPool().getHttpConnectionCount() + " SPDY: " + client.getConnectionPool().getSpdyConnectionCount());
