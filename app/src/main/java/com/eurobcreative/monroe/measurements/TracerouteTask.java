...
import com.eurobcreative.monroe.PreemptibleMeasurementTask;
...
public class TracerouteTask extends MeasurementTask implements PreemptibleMeasurementTask {
    ...
    public static class TracerouteDesc extends MeasurementDesc {
        ...
        public TracerouteDesc(String key, Date startTime, Date endTime, double intervalSec, long count,
            ...
            super(TracerouteTask.TYPE, key, startTime, endTime, intervalSec, count, priority, contextIntervalSec, params);
        ...
        protected void initializeParams(Map<String, String> params) {
            ...
            try {
                ...
                if ((val = params.get("ping_timeout_sec")) != null && val.length() > 0 && Integer.parseInt(val) > 0) {
                    this.pingTimeoutSec = Integer.parseInt(val);
                } else {
                    this.pingTimeoutSec = TracerouteTask.DEFAULT_PING_TIMEOUT;
                }
                if ((val = params.get("ping_interval_sec")) != null && val.length() > 0 && Integer.parseInt(val) > 0) {
                    this.pingIntervalSec = Integer.parseInt(val);
                } else {
                    this.pingIntervalSec = Config.DEFAULT_INTERVAL_BETWEEN_ICMP_PACKET_SEC;
                }
                if ((val = params.get("pings_per_hop")) != null && val.length() > 0 && Integer.parseInt(val) > 0) {
                    this.pingsPerHop = Integer.parseInt(val);
                } else {
                    this.pingsPerHop = TracerouteTask.DEFAULT_PINGS_PER_HOP;
                }
                if ((val = params.get("max_hop_count")) != null && val.length() > 0 && Integer.parseInt(val) > 0) {
                ...
                if ((val = params.get("parallel_probe_num")) != null && val.length() > 0 && Integer.parseInt(val) > 0) {
        ...
        public static final Parcelable.Creator<TracerouteDesc> CREATOR = new Parcelable.Creator<TracerouteDesc>() {
    ...
    public static final Parcelable.Creator<TracerouteTask> CREATOR = new Parcelable.Creator<TracerouteTask>() {
    ...
    public MeasurementTask clone() {
        ...
        TracerouteDesc newDesc = new TracerouteDesc(desc.key, desc.startTime, desc.endTime, desc.intervalSec,
    ...
    public MeasurementResult[] call() throws MeasurementError {
        ...
        CompletionService<HopInfo> taskCompletionService = new ExecutorCompletionService<>(hopExecutorService);
        ...
        */
        ...
        for (int i = maxHopCount; i > 0; i--) {
            ...
            String command = Util.constructCommand(task.pingExe, "-n", "-t", ttl, "-s", task.packetSizeByte, "-c 1", target);
    ...
    class HopExecutor implements Callable {
        ...
        public HopInfo call() {
            double rtt = 0;
            HashSet<String> hostsAtThisDistance = new HashSet<>();
            try {
