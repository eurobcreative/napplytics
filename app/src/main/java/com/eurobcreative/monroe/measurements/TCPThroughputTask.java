...
public class TCPThroughputTask extends MeasurementTask {
    ...
    private ArrayList<Double> samplingResults = new ArrayList<>();
    ...
    public TCPThroughputTask(MeasurementDesc desc) {
        super(new TCPThroughputDesc(desc.key, desc.startTime, desc.endTime, desc.intervalSec, desc.count,
                desc.priority, desc.contextIntervalSec, desc.parameters));
        this.taskProgress = TaskProgress.FAILED;
        this.stopFlag = false;
        this.duration = (long) (this.KSEC * ((TCPThroughputDesc) measurementDesc).duration_period_sec +
    ...
    public static final Parcelable.Creator<TCPThroughputTask> CREATOR = new Parcelable.Creator<TCPThroughputTask>() {
    ...
    public static class TCPThroughputDesc extends MeasurementDesc {
        ...
        public TCPThroughputDesc(String key, Date startTime, Date endTime, double intervalSec, long count,
                                 long priority, int contextIntervalSec, Map<String, String> params) throws InvalidParameterException {
            super(TCPThroughputTask.TYPE, key, startTime, endTime, intervalSec, count, priority, contextIntervalSec, params);
        ...
        public static final Parcelable.Creator<TCPThroughputDesc> CREATOR = new Parcelable.Creator<TCPThroughputDesc>() {
        ...
        protected void initializeParams(Map<String, String> params) {
            ...
            try {
                String readVal;
                if ((readVal = params.get("data_limit_mb_down")) != null && readVal.length() > 0 && Integer.parseInt(readVal) > 0) {
                ...
                if ((readVal = params.get("data_limit_mb_up")) != null && readVal.length() > 0 && Integer.parseInt(readVal) > 0) {
                ...
                if ((readVal = params.get("duration_period_sec")) != null && readVal.length() > 0 && Integer.parseInt(readVal) > 0) {
                ...
                if ((readVal = params.get("pkt_size_up_bytes")) != null && readVal.length() > 0 && Integer.parseInt(readVal) > 0) {
                ...
                if ((readVal = params.get("sample_period_sec")) != null && readVal.length() > 0 && Integer.parseInt(readVal) > 0) {
                ...
                if ((readVal = params.get("slow_start_period_sec")) != null && readVal.length() > 0 && Integer.parseInt(readVal) > 0) {
                ...
                if ((readVal = params.get("tcp_timeout_sec")) != null && readVal.length() > 0 && Integer.parseInt(readVal) > 0) {
        ...
        public double calMedianSpeedFromTCPThroughputOutput(String outputInJSON) {
            ...
            double result;
            if (resultLen % 2 == 0) {
                result = (Double.parseDouble(splitResult[resultLen / 2]) + Double.parseDouble(splitResult[resultLen / 2 - 1])) / 2;
            } else {
    ...
    public MeasurementTask clone() {
        ...
        TCPThroughputDesc newDesc = new TCPThroughputDesc(desc.key, desc.startTime, desc.endTime,
    ...
    public MeasurementResult[] call() throws MeasurementError {
        ...
        if (!desc.target.equals(MLabNS.TARGET)) {
            ...
            throw new InvalidParameterException("Unknown target " + desc.target + " for TCPThroughput");
        }
    ...
    private boolean acquireServerConfig() throws MeasurementError, IOException, InterruptedException {
        ...
        try {
            ...
            buildUpSocket(tcpSocket, ((TCPThroughputDesc) measurementDesc).target, TCPThroughputTask.PORT_CONFIG);
            iStream = tcpSocket.getInputStream();
        } catch (IOException e) {
            throw new MeasurementError("Error open uplink socket at " + ((TCPThroughputDesc) measurementDesc).target +
                    " with port " + TCPThroughputTask.PORT_CONFIG);
        }
        ...
        try {
            ...
            throw new MeasurementError("Error to acquire configuration from " + ((TCPThroughputDesc) measurementDesc).target);
        } finally {
            iStream.close();
            tcpSocket.close();
    ...
    private void uplink() throws MeasurementError, IOException, InterruptedException {
        ...
        try {
            tcpSocket = new Socket();
            buildUpSocket(tcpSocket, ((TCPThroughputDesc) measurementDesc).target, TCPThroughputTask.PORT_UPLINK);
            oStream = tcpSocket.getOutputStream();
            iStream = tcpSocket.getInputStream();
        } catch (IOException e) {
            e.printStackTrace();
            throw new MeasurementError("Error open uplink socket at " + ((TCPThroughputDesc) measurementDesc).target +
                    " with port " + TCPThroughputTask.PORT_UPLINK);
        }

        long startTime = System.currentTimeMillis();
        long endTime;
        int data_limit_byte_up = (int) (((TCPThroughputDesc) measurementDesc).data_limit_mb_up * this.KBYTE * this.KBYTE);
        byte[] uplinkBuffer = new byte[((TCPThroughputDesc) measurementDesc).pkt_size_up_bytes];
        this.genRandomByteArray(uplinkBuffer);

        try {
            long totalDuration = (long) (this.KSEC * ((TCPThroughputDesc) measurementDesc).duration_period_sec + ((TCPThroughputDesc) measurementDesc).slow_start_period_sec);
            do {
                if (stopFlag) {
                    throw new MeasurementError("Cancelled");
                }
                ...
                if (this.DATA_LIMIT_ON && this.totalSendSize >= data_limit_byte_up) {
                    Logger.i("Detect uplink exceeding limitation " + ((TCPThroughputDesc) measurementDesc).data_limit_mb_up + " MB");
            ...
            Logger.i("Uplink total data comsumption is " + (double) this.totalSendSize / (1024 * 1024) + " MB");
            ...
            throw new MeasurementError("Error to send/receive data to " + ((TCPThroughputDesc) measurementDesc).target);
        } finally {
            iStream.close();
            oStream.close();
            tcpSocket.close();
    ...
    private void downlink() throws MeasurementError, IOException {
        ...
        try {
            tcpSocket = new Socket();
            buildUpSocket(tcpSocket, ((TCPThroughputDesc) measurementDesc).target, TCPThroughputTask.PORT_DOWNLINK);
            iStream = tcpSocket.getInputStream();
        } catch (IOException i) {
            Logger.e("Downlink socket opening error" + i.getCause().toString());
            throw new MeasurementError("Error to open downlink socket at " + ((TCPThroughputDesc) measurementDesc).target +
                    " with port " + TCPThroughputTask.PORT_DOWNLINK);
        }
        try {
            int read_bytes;

            int data_limit_byte_down = (int) (this.KBYTE * this.KBYTE * ((TCPThroughputDesc) measurementDesc).data_limit_mb_down);
            byte[] buffer = new byte[this.BUFFER_SIZE];
            long totalDuration = (long) (this.KSEC * ((TCPThroughputDesc) measurementDesc).duration_period_sec +
                    ((TCPThroughputDesc) measurementDesc).slow_start_period_sec);
            ...
            this.taskDuration = (System.currentTimeMillis() - (double) this.taskStartTime) / 1000.0;
