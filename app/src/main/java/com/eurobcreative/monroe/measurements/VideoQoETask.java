...
import com.eurobcreative.monroe.util.video.VideoPlayerService;
...
public class VideoQoETask extends MeasurementTask {
    ...
    private ArrayList<Double> rebufferTimes = new ArrayList<>();
    private ArrayList<String> goodputTimestamps = new ArrayList<>();
    private ArrayList<Double> goodputValues = new ArrayList<>();
    private ArrayList<Long> goodputEstimateValues = new ArrayList<>();
    private ArrayList<String> bitrateTimestamps = new ArrayList<>();
    private ArrayList<Integer> bitrateValues = new ArrayList<>();
    ...
    public static class VideoQoEDesc extends MeasurementDesc {
        ...
        public VideoQoEDesc(String key, Date startTime, Date endTime, double intervalSec, long count,
                            long priority, int contextIntervalSec, Map<String, String> params) {
            super(VideoQoETask.TYPE, key, startTime, endTime, intervalSec, count, priority, contextIntervalSec, params);
            initializeParams(params);
            if (this.contentURL == null) {
                throw new InvalidParameterException("Video QoE task cannot be created due to null video url string");
            }
            if (this.contentType != DemoUtil.TYPE_DASH_VOD && this.contentType != DemoUtil.TYPE_PROGRESSIVE && this.contentType != DemoUtil.TYPE_BBA) {
                throw new InvalidParameterException("Video QoE task cannot be created due to invalid streaming algorithm: " + this.contentType);
            }
        }
        ...
        public static final Parcelable.Creator<VideoQoEDesc> CREATOR = new Parcelable.Creator<VideoQoEDesc>() {
    ...
    public VideoQoETask(MeasurementDesc desc) {
        super(new VideoQoEDesc(desc.key, desc.startTime, desc.endTime, desc.intervalSec, desc.count,
    ...
    public static final Parcelable.Creator<VideoQoETask> CREATOR = new Parcelable.Creator<VideoQoETask>() {
    ...
    public MeasurementTask clone() {
        ...
        VideoQoEDesc newDesc = new VideoQoEDesc(desc.key, desc.startTime, desc.endTime, desc.intervalSec,
        ...
        BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (intent.hasExtra(UpdateIntent.VIDEO_TASK_PAYLOAD_BITRATE_TIMESTAMP)) {
                    ...
                    bitrateTimestamps = new ArrayList<>(Arrays.asList(bitrateTimestampArray));
