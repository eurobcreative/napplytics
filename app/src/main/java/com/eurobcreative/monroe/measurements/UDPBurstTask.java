...
public class UDPBurstTask extends MeasurementTask {
    ...
    public static class UDPBurstDesc extends MeasurementDesc {
        ...
        public UDPBurstDesc(String key, Date startTime, Date endTime, double intervalSec, long count,
            ...
            super(UDPBurstTask.TYPE, key, startTime, endTime, intervalSec, count, priority, contextIntervalSec, params);
        ...
        protected void initializeParams(Map<String, String> params) {
            ...
            try {
                ...
                if ((val = params.get("packet_size_byte")) != null && val.length() > 0 && Integer.parseInt(val) > 0) {
                ...
                if ((val = params.get("packet_burst")) != null && val.length() > 0 && Integer.parseInt(val) > 0) {
                    this.udpBurstCount = Integer.parseInt(val);
                    if (this.udpBurstCount > MAX_BURSTCOUNT) {
                        this.udpBurstCount = MAX_BURSTCOUNT;
                    }
                }
                if ((val = params.get("udp_interval")) != null && val.length() > 0 && Integer.parseInt(val) >= 0) {
        ...
        public static final Parcelable.Creator<UDPBurstDesc> CREATOR = new Parcelable.Creator<UDPBurstDesc>() {
    ...
    public static final Parcelable.Creator<UDPBurstTask> CREATOR = new Parcelable.Creator<UDPBurstTask>() {
    ...
    public MeasurementTask clone() {
        ...
        UDPBurstDesc newDesc = new UDPBurstDesc(desc.key, desc.startTime, desc.endTime, desc.intervalSec,
    ...
    private UDPResult recvUpResponse(DatagramSocket sock) throws MeasurementError {
        ...
        if (responsePacket.type == PKT_RESPONSE) {
            ...
            if (responsePacket.seq != seq) {
                Logger.e("Error: Server send response packet with different seq, old " + seq + " => new " + responsePacket.seq);
            }
            ...
            Logger.i("Recv UDP resp from " + desc.target + " type:" + responsePacket.type + " burst:"
                    ...
                    + " out_of_order_num: " + responsePacket.outOfOrderNum + " jitter: " + responsePacket.timestamp);
        ...
        for (int i = 0; i < desc.udpBurstCount; i++) {
            ...
            if (dataPacket.type == UDPBurstTask.PKT_DATA) {
                ...
                if (dataPacket.seq != seq) {
                    String err = "Server send data packets with different seq, old " + seq + " => new " + dataPacket.seq;
                    Logger.e(err);
                    throw new MeasurementError(err);
                }

                Logger.i("Recv UDP response from " + desc.target + " type:" + dataPacket.type + " burst:"
                        + dataPacket.burstCount + " pktnum:" + dataPacket.packetNum + " timestamp:" + dataPacket.timestamp);
    ...
    public MeasurementResult[] call() throws MeasurementError {
        ...
        try {
            ...
            // Hongyi: we should update seq number no matter the measurement is succeeded or not.
            // It ensures previous last UDP burst's packets will not affect the current one.
            seq++;
            if (socket != null) {
                socket.close();
            }
        ...
        MeasurementResult result = new MeasurementResult(phoneUtils.getDeviceInfo().deviceId, phoneUtils.getDeviceProperty(this.getKey()),
                UDPBurstTask.TYPE, System.currentTimeMillis() * 1000, this.taskProgress, this.measurementDesc);
